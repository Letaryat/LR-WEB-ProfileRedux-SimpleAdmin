...
class Player
{
    ...
    function __construct($General, $Db, $Translate, $id, $sg, $search)
    {
        ...
        empty($this->found[$this->server_group]) && get_iframe('009', '?????? ???????? ?? ??????????');
     ...
     * @since 0.2
     *
     * @param string       $steamid3  Steam ID 3 ??????.
     *
     * @return int                    ??????? ???? ???????????.
     */
    ...
    public function get_db_plugin_rankxp()
    {
        switch ($this->found[$this->server_group]['DB_mod']) {
            case 'LevelsRanks':
                if ($this->Db->mysql_table_search('LevelsRanks', $this->found[$this->server_group]['USER_ID'], $this->found[$this->server_group]['DB'], $this->found[$this->server_group]['Table'] . '_ranks') == 1) :
                    $ranks = $this->Db->queryAll('LevelsRanks', $this->found[$this->server_group]['USER_ID'], $this->found[$this->server_group]['DB'], "SELECT `value` FROM `" . $this->found[$this->server_group]['Table'] . "_ranks` ORDER BY `value` ASC");
    ...
    public function get_db_SBSId()
    {
        if (!empty($this->Db->db_data['SourceBans'])) :
            return $this->Db->query('SourceBans', $this->Db->db_data['SourceBans'][0]['USER_ID'], $this->Db->db_data['SourceBans'][0]['DB_num'], "SELECT `sa_servers`.`id` FROM `sa_servers` WHERE `sa_servers`.`address` = '" . $this->lws['server_ip_explode'][0] . $this->lws['server_ip_explode'][1] . "'");
        endif;
    }
    ...
    public function get_db_SBBans()
    {
        if (!empty($this->Db->db_data['SourceBans'])) {
                return $this->Db->queryAll('SourceBans', $this->Db->db_data['SourceBans'][0]['USER_ID'], $this->Db->db_data['SourceBans'][0]['DB_num'], "SELECT 
                    `sa_bans`.`admin_steamid`,
                    `sa_bans`.`admin_name`,
                    `sa_bans`.`player_steamid`,
                    `sa_bans`.`player_name`,
                    `sa_bans`.`created`,
                    `sa_bans`.`duration`,
                    `sa_bans`.`ends`,
    ...
    public function get_db_SBComms()
    {
        if (!empty($this->Db->db_data['SourceBans'])) {
            return $this->Db->queryAll('SourceBans', $this->Db->db_data['SourceBans'][0]['USER_ID'], $this->Db->db_data['SourceBans'][0]['DB_num'], "SELECT 
                `sa_mutes`.`admin_steamid`,
                `sa_mutes`.`admin_name`,
                `sa_mutes`.`player_steamid`,
                `sa_mutes`.`player_name`,
                `sa_mutes`.`created`,
                `sa_mutes`.`duration`,
                `sa_mutes`.`ends`,
                `sa_mutes`.`reason`,
    ...
    public function get_db_SBAdmins()
    {
        if (!empty($this->Db->db_data['SourceBans'])) {
                return $this->Db->queryAll('SourceBans', $this->Db->db_data['SourceBans'][0]['USER_ID'], $this->Db->db_data['SourceBans'][0]['DB_num'], "SELECT 
                    ...
                    FROM `sa_admins`
                    WHERE `player_steamid` = '". $this->get_steam_64() ."'");
        }
    }
    ...
    public function get_db_Vips()
    {
        if (!empty($this->Db->db_data['Vips'])) {
            return $this->Db->queryAll('Vips', $this->Db->db_data['Vips'][0]['USER_ID'], $this->Db->db_data['Vips'][0]['DB_num'], "SELECT 
                `sklepcs_vip`.`identity`,
                `sklepcs_vip`.`koniec`
                FROM `sklepcs_vip`
                WHERE `identity` = '". $this->get_steam_32() ."' ORDER BY `koniec` DESC");
    }
    }
