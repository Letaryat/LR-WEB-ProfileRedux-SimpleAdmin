                        ...
                        <span class="check_ban">
                            ...
                            <?php
                                if ($player_ban['end'] == 0){ 
                                    ...
                                    echo '<a class="special-orange">'. $Modules->action_time_exchange(($player_ban['created'] + $player_ban['ends']) - time()) .'</a>';
                                }
                            ?>
                        </span>
                        <br />
            ...
            <ul class="bans_list_body">
                <?php for ($i = 0, $sz = sizeof($res); $i < $sz; $i++) :
                    ...
                    <li class="pointer">
                        ...
                        <span class="bans_banned_user" onclick="location.href= '<?= $General->arr_general['site'] ?>profiles/<?= con_steam32to64($res[$i]['steamid64']) ?>/?search=1'"><?php echo action_text_clear(action_text_trim($res[$i]['player_name'], 13)) ?></span>
                        ...
                        <span>
                            <?php
                                if ($res[$i]['ends'] == 0){ 
                                    ...
                                    echo '<div class="color-gray">'. $Modules->action_time_exchange($res[$i]['ends'] - $res[$i]['created']) ."</div>";
                                }
                            ?>
                        </span>
                        <span>
                            <?php
							    if(!empty($res[$i]['reason']) && (($res[$i]['end'] != 0 && time() < ($res[$i]['end'] - $res[$i]['created']) + $res[$i]['end']) || ($res[$i]['end'] == 0))){
							    	echo '<div data-tippy-placement="bottom" data-tippy-content="By: '. $res[$i]["admin_name"] .'<br/>Reason: '. $res[$i]['reason'] .'" class="color-blue" style="color: var(--ct-color);">'. $Translate->get_translate_module_phrase('module_page_bans', '_Unban') .'</div>'; 
                                } elseif ($res[$i]['end'] == 0){ 
							    	echo '<div class="color-red">' . $Translate->get_translate_module_phrase('module_page_profiles', '_Never') . '</div>';
							    } else { 
							    	if ($res[$i]['ban_active'] == 0 || ($res[$i]['end'] - time()) <= 0){ 
							    		echo '<div class="color-green"><strike>'. $Translate->get_translate_module_phrase('module_page_bans', '_BanExpired') .'</strike></div>'; 
							    	} else { 
							    		echo '<div class="color-gray">'. $Modules->action_time_exchange($res[$i]['end'] - time()) ."</div>";
							    	} 
							    }
							?>
                        </span>
